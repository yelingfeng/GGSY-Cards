...
async function bundle(options?: BuildOptions) {
  await build({
    outfile: `${cwd()}/dist/es/ggsy-cards.esm.js`,
    bundle: true,
    entryPoints: [`${cwd()}/src/packages/ggsy-cards.ts`],
    plugins: [vue()],
    loader: { '.png': 'dataurl' },
    external: ['vue', 'ggsy-cards/*', '@vue/*'],
    format: 'esm',
    minify: true,
    ...options,
  })
...
async function combineCss() {
  ...
  await Promise.all([
    fs.promises.writeFile(
      `${cwd()}/dist/es/ggsy-cards.esm.css`,
      content
    ),
    fs.promises.writeFile(
      `${cwd()}/dist/lib/ggsy-cards.umd.css`,
      content
    ),
  ])

  const name = 'ggsy-cards.min.css'
  await Promise.all([
    fs.promises.rename(
      `${cwd()}/dist/es/ggsy-cards.esm.css`,
      `${cwd()}/dist/es/${name}`
